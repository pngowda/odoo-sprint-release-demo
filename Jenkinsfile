...
pipeline {
    ...
    stages {
        ...
        stage('Python Lint, Format & Security') {
            steps {
                sh '''#!/bin/bash
                    # Convert space-separated MODULES to comma-separated list
                    MODULE_LIST="${MODULES// /,}"

                    # Run linters and formatters with output
                    echo "Running flake8..."
                    flake8 $MODULE_LIST | tee flake8-report.txt || true

                    echo "Checking formatting with black..."
                    black --check $MODULE_LIST | tee black-report.txt || true

                    echo "Auto-formatting with black..."
                    black $MODULE_LIST > black-formatting.txt 2>&1 || true

                    echo "Running security scan with bandit..."
                    bandit -r $MODULE_LIST | tee bandit-report.txt || true
                '''
                archiveArtifacts artifacts: '*.txt', allowEmptyArchive: true // Archive Python lint report
            }
        }
        stage('Shell Lint & Format') {
            steps {
                sh '''#!/bin/bash
                    ...
                    find $MODULES -name "*.sh" -exec shellcheck {} + | tee shellcheck-report.txt || true

                    echo "Checking shell formatting with shfmt..."
                    find $MODULES -name "*.sh" -exec shfmt -d -i 4 -ci {} + | tee shfmt-report.txt || true

                    echo "Applying formatting with shfmt..."
                    find $MODULES -name "*.sh" -exec shfmt -w -i 4 -ci {} +
                '''
                archiveArtifacts artifacts: '*.txt', allowEmptyArchive: true // Archive Shell lint report
            }
        }
        stage('Trivy Scan') {
            steps {
